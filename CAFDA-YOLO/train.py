...
class Trainer(object):
    ...
    def train(self):
        ...
        writer = SummaryWriter( 
            logdir='../logs/{}-Epoch_{}-Batch_{}_{}'.format(start_time, self.epochs, cfg.TRAIN['BATCH_SIZE'],cfg.TRAIN['TRAIN_IMG_SIZE']))
        create_dir('../files/{}-Epoch_{}-Batch_{}_{}'.format(start_time, self.epochs, cfg.TRAIN['BATCH_SIZE'],cfg.TRAIN['TRAIN_IMG_SIZE']))
        self.weight_path = '../files/{}-Epoch_{}-Batch_{}_{}'.format(start_time, self.epochs, cfg.TRAIN['BATCH_SIZE'],cfg.TRAIN['TRAIN_IMG_SIZE'])
        print(self.weight_path)
        
        self.map_log_path = os.path.join(self.weight_path, 'mAP_log.txt')
        with open(self.map_log_path, 'w') as f:
            f.write("Epoch\tClass\tAP\n")  
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            with tqdm(total=len(self.train_dataloader), unit='batch', ncols=120,
                ...
                for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes) in enumerate(
                    ...
                    with torch.amp.autocast('cuda', enabled=self.fp_16):
                ...
                with torch.no_grad():
                    ...
                    with open(self.map_log_path, 'a') as f:
                        for class_name, ap in aps.items():
                            print('[{}/{} Epoch] Validating: {} AP: {:.4f}'.format(epoch + 1, self.epochs, class_name, ap))
                            valid_map += ap
                            f.write(f"{epoch + 1}\t{class_name}\t{ap:.4f}\n")
                        valid_map = valid_map / self.classes_num
                        f.write(f"{epoch + 1}\tmAP\t{valid_map:.4f}\n")
                    print(
...
if __name__ == '__main__':
    ...
    parser.add_argument('-g', '--gpu_id', type=int, default=1, help='whither use GPU(0) or CPU(-1)')
