...
def fits_2_npy(fits_path: str, bbox: list, fits_name: str, npy_save_path: str, img_save_path: str, label_save_path: str,
               output_size: int = 256, padding: int = 10, window_number: int = 5) -> List[Dict]:
    """
    Convert fits to npy
    :param npy_save_path:
    :param img_save_path:
    ...
    if box_x >= output_size or box_y >= output_size:
        print(f"[Warning] Skipping processing for {fits_name} due to invalid bbox size: {bbox}")
        return []
...
if __name__ == '__main__':
    ...
    JPEG_save_path = config['JPEG_save_path'] if cross_index is None or cross_index == '' else config[
        'JPEG_save_path'].replace('bss_dataset', 'bss_dataset_{}'.format(cross_index))
    txt_save_path = config['txt_save_path'] if cross_index is None or cross_index == '' else config[
        'txt_save_path'].replace('bss_dataset', 'bss_dataset_{}'.format(cross_index))
    annotation_save_path = config['annotation_save_path'] if cross_index is None or cross_index == '' else \
        config[
            'annotation_save_path'].replace('bss_dataset', 'bss_dataset_{}'.format(cross_index))
    img_save_path = config['img_save_path'] if cross_index is None or cross_index == '' else config[
        'img_save_path'].replace('bss_dataset', 'bss_dataset_{}'.format(cross_index))
    ...
    with open(os.path.join(txt_save_path, 'bss_{}.txt'.format(model)), 'w') as f:
